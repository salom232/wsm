$(document).ready(function() {
    ...
    axios.post(`${URI}/Log/`, {
        ...
        data: {
            ...
            ACTIONDESC: 'Entro a sección ACTUALIZACION MASIVA.'
            }
    });
});
...
function ProcessFile() {
  ...
  if (regex.test(fileUpload.value.toLowerCase())) {
    ...
    $('#MessageModal').modal('toggle');
    $('#MessageModal').modal('show');
    $('#ModalTitle').text('Tamaño de peso excedido.');
    $('#ModalBody').text('El archivo pesa mas de 2 MB, favor de elegir otro.');
...
function sendExcelObjectToApi(JSONObject){
    if($("#ItemType option:selected").val() === 'NV'){
        ...
        jsonDataObject.BinaryData = JSONObject;
        jsonDataObject.Supplier = sessionStorage.getItem('UserFName');
        jsonDataObject.SupplierID = sessionStorage.getItem('UserIDProv');
        jsonDataObject.ItemType = jsonDataObject.ItemType = $("#ItemType option:selected").val();
        ...
        $.ajax({
            ...
            type: "POST",
            crossDomain: true,
            url: `${URI}/postUploadExcelDataToAkeneoMUJSON/`,
            contentType: 'application/json',
            data: jsonData,
            async: true,
            success: function (data){
